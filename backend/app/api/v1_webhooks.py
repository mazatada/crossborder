...
def list_dlq():
    ...
    dlq_entries = db.session.query(WebhookDLQ).filter_by(replayed=False).order_by(WebhookDLQ.created_at.desc()).all()
...
def replay_dlq(dlq_id: int):
    ...
    if not dlq_entry:
        return jsonify({"status": "error", "error": {"code": "NOT_FOUND", "message": "DLQ entry not found"}}), 404
    
    if dlq_entry.replayed:
        return jsonify({"status": "error", "error": {"code": "ALREADY_REPLAYED", "message": "This entry has already been replayed"}}), 400
    
    # Get webhook
    webhook = db.session.query(WebhookEndpoint).filter_by(id=dlq_entry.webhook_id).first()
    if not webhook:
        return jsonify({"status": "error", "error": {"code": "NOT_FOUND", "message": "Webhook not found"}}), 404
    ...
    if trace_id:
        record_event(event="DLQ_REPLAYED", trace_id=trace_id, dlq_id=dlq_id, success=result.get("success"))
...
def cleanup_dlq():
    ...
    now = datetime.utcnow()
    expired_entries = db.session.query(WebhookDLQ).filter(WebhookDLQ.expires_at < now).all()
