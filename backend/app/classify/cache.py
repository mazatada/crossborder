...
class HSCache:
    ...
    def __init__(self, backend: Optional[CacheBackend] = None):
        if backend:
            ...
            try:
                ...
                logger.warning(f"Failed to connect to Redis ({e}), falling back to InMemoryCache")
    ...
    def generate_cache_key(self, product_data: Dict, rules_version: str) -> str:
        ...
        normalized = {
            ...
            "origin": product_data.get("origin_country", "").upper() if product_data.get("origin_country") else "",
            "ingredients": limited_ingredients,
            "process": sorted([str(p).lower() for p in product_data.get("process", [])[:5]]),  # 5個まで
