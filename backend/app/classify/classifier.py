...
class HSClassifier:
    ...
    def classify(self, product_data: Dict[str, Any]) -> Dict:
        ...
        logger.info(f"Matched {len(rule_matches)} rules for product: {product_data.get('name', 'unknown')}")
    ...
    def _generate_rationale(self, matches: List[Dict], product_data: Dict) -> List[str]:
        ...
        if ingredients:
            ...
            if sorted_ingredients:
                ...
                # 含有率なしの場合
                primary_ids = [ing.get("id") for ing in ingredients[:3] if isinstance(ing, dict)]
                if primary_ids:
