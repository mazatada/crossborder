...
def test_update_product(client, api_key_header):
    res1 = client.post(
        "/v1/products", json={"title": "To Update"}, headers=api_key_header
    )
    pid = res1.get_json()["id"]

    res2 = client.put(
        f"/v1/products/{pid}",
        json={"title": "Updated Title", "unit_weight_g": 100},
...
def test_validate_product_missing_fields(client, api_key_header):
    payload = {"title": "Test Invalid", "is_food": False}
    res1 = client.post("/v1/products", json=payload, headers=api_key_header)
    pid = res1.get_json()["id"]
...
def test_update_product_immutability(client, api_key_header):
    ...
    res2 = client.put(
        f"/v1/products/{pid}", json={"is_food": True}, headers=api_key_header
    )
    ...
    res3 = client.put(
        f"/v1/products/{pid}", json={"title": "Safe Update"}, headers=api_key_header
    )
