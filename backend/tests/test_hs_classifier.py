...
class TestRuleEngine:
    ...
    def test_contains_any_ids(self):
        ...
        assert engine._contains_any_ids(ingredients, ["ing_sugar", "ing_cocoa"]) == True
        assert engine._contains_any_ids([], ["ing_wheat_flour"]) == False
        assert engine._contains_any_ids(None, ["ing_wheat_flour"]) == False
    ...
    def test_process_any(self):
        ...
        assert engine._process_any(processes, ["baking"]) == True
        assert engine._process_any(processes, ["roasting"]) == False
        assert engine._process_any(processes, ["BAKING"]) == True  # 大文字小文字無視
        assert engine._process_any([], ["baking"]) == False
        assert engine._process_any(None, ["baking"]) == False
    
    def test_origin_in(self):
        """origin_in述語のテスト"""
        engine = RuleEngine()
        
        assert engine._origin_in("JP", ["JP", "US"]) == True
        assert engine._origin_in("jp", ["JP", "US"]) == True  # 大文字小文字無視
        assert engine._origin_in("CN", ["JP", "US"]) == False
        assert engine._origin_in(None, ["JP"]) == False
    
    def test_category_is(self):
        """category_is述語のテスト"""
        engine = RuleEngine()
        
        assert engine._category_is("confectionery", "confectionery") == True
        assert engine._category_is("Confectionery", "confectionery") == True  # 大文字小文字無視
        assert engine._category_is("beverages", "confectionery") == False
        assert engine._category_is(None, "confectionery") == False
    ...
    def test_not_contains_ids(self):
        ...
        assert engine._not_contains_ids(ingredients, ["ing_cocoa"]) == True
        assert engine._not_contains_ids(ingredients, ["ing_wheat_flour"]) == False
        assert engine._not_contains_ids([], ["ing_cocoa"]) == True
    ...
    def test_ingredient_pct_threshold(self):
        ...
        # 最小値のみ
        assert engine._ingredient_pct_threshold(ingredients, "ing_wheat_flour", 30.0) == True
        assert engine._ingredient_pct_threshold(ingredients, "ing_wheat_flour", 40.0) == False
        
        # 範囲指定
        assert engine._ingredient_pct_threshold(ingredients, "ing_sugar", 20.0, 30.0) == True
        assert engine._ingredient_pct_threshold(ingredients, "ing_sugar", 30.0, 40.0) == False
        
        # 存在しない成分
        assert engine._ingredient_pct_threshold(ingredients, "ing_cocoa", 10.0) == False
    
    def test_always(self):
        """always述語のテスト"""
        engine = RuleEngine()
        assert engine._always() == True
...
class TestHSClassifier:
    ...
    def test_classify_high_wheat_content(self):
        ...
        high_wheat_rule = next(
            (m for m in result["rule_matches"] if m["rule_id"] == "hs_food_008"),
            None
        )
