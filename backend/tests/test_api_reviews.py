...
def test_assign_hs_review(client, api_key_header, db_session):
    ...
    res = client.post(
        f"/v1/reviews/hs/{record.id}/assign",
...
def test_finalize_hs_review(client, api_key_header, db_session):
    product = Product(
        title="Test Product for Finalize",
        description_en="desc",
        origin_country="JP",
        is_food=False,
        status="draft",
    ...
    res = client.post(
        f"/v1/reviews/hs/{record.id}/finalize",
        json={"final_hs_code": "9999.99"},
        headers=api_key_header,
    )
    assert res.status_code == 200
    data = res.get_json()
